...
<div class="container mx-auto px-4 py-6 max-w-4xl">
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Gerenciamento de Afastamentos</h1>
        ...
        <a href="{{route('manageEmployee', ['id'=>$employment_bond->id])}}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded text-sm font-bold">
            Voltar
        </a>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow mb-6">
        ...
        <form action="{{ route('leaves.store', $employment_bond->id) }}" method="POST">
            @csrf
            ...
            <div class="control">
              <div class="select w-full">
                <select name="type" class="w-full">
                  ...
                  <option value="medical">LICENÇA MÉDICA</option>
                  <option value="vacation">FÉRIAS</option>
                  <option value="recess">RECESSO</option>
                  <option value="off_tre">FOLGA TRE</option>
                </select>
              </div>
            </div>
          </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="description" placeholder="Ex: Tratamento de saúde prolongado" class="w-full border rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow">
        <h3 class="text-lg font-bold text-gray-700 mb-4 border-b pb-2">Histórico de Afastamentos Cadastrados</h3>

        @if($employment_bond->leaves->isEmpty())
            <p class="text-gray-500 italic">Nenhum Afastamento registrado para este vínculo até o momento.</p>
        @else
                    ...
                    <tbody>
                        @foreach($employment_bond->leaves as $leave)
                            <tr class="hover:bg-gray-50 text-sm">
                                ...
                                <td class="border border-gray-200 p-3">{{ $leave->type_name }}</td>
                                <td class="border border-gray-200 p-3">{{ $leave->description ?? '-' }}</td>
                                <td class="border border-gray-200 p-3 text-center">
                                    <form action="{{ route('leaves.destroy', $leave->id) }}" method="POST" onsubmit="return confirm('Deseja realmente excluir este registro de afastamento?');">
                                        @csrf
                                        @method('DELETE')
